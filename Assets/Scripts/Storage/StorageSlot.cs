		...
		if (dragging) {
			...
			if (hoveredItem == null) {
				...
				if (hoveredItem.transform.parent.transform.parent.name != "DragParent" || hoveredItem.name == "Background") {
					if (slotHolder.tag == "Inventory") {
						// Move item from storage to inventory
						storage.AddToInventory(item, int.Parse(amount.text), slotNr, originalIndex);
					} else if (Inventory.instance.items[slotNr] == null) {
						// Move item to empty field
						storage.Move(originalIndex, slotNr);
					} else if (hoveredItem.name == "Trash") {
						// Destroy item
						storage.Destroy(originalIndex);
					} else {
						// Swap items
						storage.Swap(originalIndex, hoveredItem.transform.parent.transform.parent.transform.GetSiblingIndex());
					}
				}
			}
